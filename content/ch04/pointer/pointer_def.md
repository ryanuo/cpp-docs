...
根据之前[类型标识](/ch04/array/array_init#idx_类型标识)的概念，我们可以得知指针类型的类型标识是 `T*`，其中 `T` 是基类型。不过由于指针类型属于复合类型，所以在同时声明并定义多个指针时，倘若
```cpp
int* a, b;
```
这样写，只有 `a` 是一个 `int*` 类型的指针，而 `b` 只是一个 `int` 型变量。这一点和[数组的声明](/ch04/array/array_init#关于类型标识)很相似，但是不容易看出。如果你想将 `a` 和 `b` 都声明为指针，你需要
```cpp
int *a, *b;
```
...
> 关于指针声明符中 `*` 的位置，[附录](/appendix/coding_style#指针声明)中提供了更多的讨论与解释。