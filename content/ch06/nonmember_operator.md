...
然后呢，`<<` 是一个运算符。如果你阅读了[位运算符](/ch02/part2/bit_operator)这一节的话，你会知道它是“左移运算符”，在一般情况下它的操作数都是整数。
```cpp
1 << 2; // 左移运算符的一般用法
```
...
这就是非成员的运算符重载了。不过与此相比，对于 `operator+` 更好的处理是只定义非成员的 `String operator+(const String& a, const String& b);`，这样任何可以隐式转换到 `String` 的类型出现在两侧都可以调用它。你会在[总结](/ch06/summary)中看到这个版本。
