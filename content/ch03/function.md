...
你会发现这里函数的定义与数学上截然不同。实际上，计算机中函数又被称为“子程序”（Subprogram），这是因为函数其实**相当于把程序的一部分提取出来**。请看这段程序，它可以输出输入的两个数之中较大的那个。
```cpp
#include <iostream>
using namespace std;
int main() {
...
而使用函数把比较的那一部分拿出来的话，则得到了完全等价的代码。
```cpp
#include <iostream>
using namespace std;
int max(int x1, int x2) {
...
也就是说，`max` 函数将原来代码中 7 到 11 行的部分独立出来；换句话说，把这几行所代表的那段过程用 `max` 函数所表示了出来。
...
`void` 是 C++ 关键字，是**空类型**（Void type）的类型说明符，但永远不存在空类型的变量。
...
请看这个例子。这段代码中，先求出 `a` 和 `b` 的最大值并输出，又求出 `b` 和 `c` 的最大值并输出。
```cpp
#include <iostream>
using namespace std;
int main() {
...
但你会发现这两次求值的代码几乎是一样的，只有变量名不同；也就是说我们几乎把一段代码抄了两遍。如果我们把这段过程用函数来概括起来，则可以用较少的代码量就实现了原来同样的功能。
```cpp
#include <iostream>
using namespace std;
int max(int x,int y) {
...
从这个角度出发，函数所体现的用途就是：减少重复或相似的代码，提高可读性。