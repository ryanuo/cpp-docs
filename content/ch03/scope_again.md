...
> 有关一个变量存储的位置、分配内存和初始化的时机是存储期的相关知识。在[链表章节](/ch04/list/storage_duration)会有更多介绍。
...
```
就像这样。但是你会发现运行的结果貌似和我们预想的有所偏差……具体的原因我们会留到[指针章节](/ch04/pointer/pointer_and_array#数组到指针的转换)再做讲解。
