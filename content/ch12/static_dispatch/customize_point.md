...
第六章我提到过[函数对象](/ch06/special_operator_overload#idx_函数对象)，就是重载了 `operator()` 的类的对象。它在使用的时候和函数很像，都是出现在函数调用运算符的左侧；但它毕竟不是函数，所以**函数对象不会应用 ADL**。标准规定：如果当前作用域存在一个函数对象，其名字正是函数调用运算符左侧的名字，那么不进行 ADL。
