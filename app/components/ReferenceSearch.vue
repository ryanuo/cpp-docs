...
function handleKeydown(e: KeyboardEvent) {
  ...
  switch (e.key) {
    ...
    case 'Enter': {
      ...
      if (entry) {
        window.open(`/reference/zh/${entry.path}`, '_blank')
        closeModal()
      }
      break
    }
...
<template>
  <div class="reference-search-wrapper">
    <UButton variant="ghost" color="neutral" icon="i-lucide-search" class="items-center gap-2 cursor-pointer"
      @click="openModal">
        ...
        <div class="bg-white dark:bg-gray-900 flex flex-col max-h-[70vh] rounded-xl">
          <div class="sticky top-0 z-10 bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700">
            <div class="flex items-center gap-2 px-4 py-3">
              ...
              <input ref="inputRef" :value="query" type="text" placeholder="搜索函数、类、类型..."
                class="flex-1 bg-transparent text-gray-900 dark:text-gray-100 placeholder:text-gray-400 focus:outline-none"
                @input="handleSearchInput">
              <UButton variant="ghost" color="neutral" size="sm" icon="i-lucide-x" @click="closeModal" />
            </div>

            <div class="flex gap-1 px-4 pb-3">
              <UButton v-for="cat in categories" :key="cat.value"
                :variant="activeCategory === cat.value ? 'soft' : 'ghost'"
                :color="activeCategory === cat.value ? 'primary' : 'neutral'" size="sm" :icon="cat.icon"
          ...
          <div ref="listRef" class="flex-1 overflow-y-auto search-results-list">
            ...
            <div v-else-if="!query" class="flex flex-col items-center justify-center py-12 text-gray-500">
            ...
            <div v-else-if="results.length === 0" class="flex flex-col items-center justify-center py-12 text-gray-500">
            ...
            <div v-else class="p-1">
              <a v-for="(entry, index) in results" :key="entry.path" :href="`/reference/zh/${entry.path}`"
                ...
                <UBadge v-if="activeCategory === 'all'" :color="getCategoryColor(entry.category)" variant="subtle"
                  size="sm" class="shrink-0">
                  {{ categoryMap[entry.category] }}
                </UBadge>

                <UIcon v-if="index === selectedIndex" name="i-lucide-arrow-right"
