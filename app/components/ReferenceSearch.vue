...
function goToResult(index: number) {
  ...
  if (!entry) return
  navigateTo(`/reference/zh/${entry.path}`)
  closeModal()
}
    ...
    <UModal v-model:open="modalOpen">
      <template #content>
        <div class="bg-white dark:bg-gray-900 flex flex-col max-h-[70vh] rounded-xl">
          ...
          <div class="shrink-0 border-t border-gray-200 dark:border-gray-700 px-4 py-2.5 flex items-center justify-between text-xs text-gray-400">
            <div class="flex items-center gap-3">
              <span class="flex items-center gap-1">
                <UKbd size="sm">↑</UKbd><UKbd size="sm">↓</UKbd> 导航
              </span>
              <span class="flex items-center gap-1">
                <UKbd size="sm">↵</UKbd> 打开
              </span>
              <span class="flex items-center gap-1">
